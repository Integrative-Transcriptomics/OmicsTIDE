...
function globalSelectionToPanther(organism, data, response, parentDivId, tabId){
    ...
    if(tabId === TabId.selectionIntersecting){
        ...
        molFuncDiv = "second-level-intersecting-diagrams-go-molfunc-" + id;
        bioProcDiv = "second-level-intersecting-diagrams-go-bioproc-" + id;
        cellCompDiv = "second-level-intersecting-diagrams-go-cellcomp-" + id;
    }

    if(tabId === TabId.selectionNonIntersecting){

        molFuncDiv = "second-level-non-intersecting-diagrams-go-molfunc-" + id;
        bioProcDiv = "second-level-non-intersecting-diagrams-go-bioproc-" + id;
        cellCompDiv = "second-level-non-intersecting-diagrams-go-cellcomp-" + id;
    }
...
function flattenGoResults(goResults){
    ...
    for(let result of goResults){
        ...
        tmpResult['id'] = result['term']['id'];
        tmpResult['label'] = result['term']['label'];
        tmpResult['minus_log10_fdr'] = parseFloat(-Math.log10(result.fdr));
        tmpResult['FDR'] = parseFloat(result.fdr.toPrecision(2));

        flatGoResults.push(tmpResult);
    }
...
function postQuery(url, inputList, organism, category, enrichmentTestType, correction, fdr_threshold, div, category_id, category_name, parentDivId){
    ...
    let comparison = tab.split("_")[0]
    let tabId = tab.split("_")[1];

    axios.get(url + inputList + "&" + organism + "&" + category + "&" + enrichmentTestType + "&" + correction)
        .then(function (response) {

            if(tabId === "selectionIntersecting"){
                tabId = "intersecting";
            }

            if(tabId === "selectionNonIntersecting"){
            ...
            //globalDataCopy[comparison][tabId]['go'][categoryFromId(category)] = flattenGoResults(goResults);
            
            console.log(categoryFromId(category));

            globalDataCopy['go'][categoryFromId(category)] = flattenGoResults(goResults);

            console.log(globalDataCopy);

            //updateGlobalDataSubset(currentlyChosenGlobalData, comparison, tabId, 'go', globalDataCopy['go'][categoryFromId(category)]);

            currentlyChosenGlobalData[comparison][tabId]['go'][[categoryFromId(category)]] = globalDataCopy['go'][categoryFromId(category)]

            console.log(currentlyChosenGlobalData);
