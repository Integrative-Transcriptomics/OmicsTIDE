...
function updateCentroidBySlider(tabName, tabId){
    ...
    let globalDataCopy = JSON.parse(JSON.stringify(currentlyChosenGlobalData[comparison][tabId]));
    globalDataCopy['data'] = updateSlide(globalDataCopy['data'], tabName, tabId);

    updateGlobalDataSubset(currentlyChosenGlobalData, comparison, tabId, 'data', globalDataCopy['data']);

    console.log(currentlyChosenGlobalData);

    // new copy
    globalDataCopy['data'] = combineLinkSpecificGlobalData2(globalDataCopy['data']);
    globalDataCopy['data'] = globalDataCopy['data'].filter(function(d) { return d.highlighted});

    if(tabId === TabId.intersecting){
        render(globalDataCopy, "first-level-intersecting-information-data-sankey-" + tabName, tabId, tabName);
    }

    let ds1Cluster = Array.from(new Set(globalDataCopy['data'].map(d => d.ds1_cluster))).filter(d => d !== null);
    let ds2Cluster = Array.from(new Set(globalDataCopy['data'].map(d => d.ds2_cluster))).filter(d => d !== null);

    for(let cluster of ds1Cluster){
      updateDetailDiagram(currentDetailDiagram, globalDataCopy, "ds1", cluster.split("_")[1], tabName, tabId);
    }

    for(let cluster of ds2Cluster){
      updateDetailDiagram(currentDetailDiagram, globalDataCopy, "ds2", cluster.split("_")[1], tabName, tabId);
    }
}
