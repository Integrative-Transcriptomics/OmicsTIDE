...
## Data 
OmicsTIDE uses CSV files following given formats as input. Detailed information on the data formats used by OmicsTIDE [is provided](DATAFORMATS.md). If no further information is required the user can directly run OmicsTIDE using the [test data](../static/data/test_data/).
...
The user can select between two and four abundance file for pairwise comparisons. The number of files is restricted to four to ensure a clear overview in the Data Matrix tab with max. four pairwise - i.e. six - comparisons. 
